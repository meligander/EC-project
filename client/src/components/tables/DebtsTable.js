...
const DebtsTable = ({
	...
	const selectItem = (item) => {
		if (item._id !== '') setOtherValues({ ...otherValues, selectedItem: item });
	};
									...
									!forAdmin ? (
										item.year <= year && (
											<React.Fragment key={index}>
												...
												) : (month < item.number && year === item.year) ||
												  item._id === '' ? (
													<td></td>
												) : (
													<td className={item.expired ? 'debt' : ''}>
														{item.value}
													</td>
												)}
											</React.Fragment>
											...
											{item.value === 0 ? (
												...
												<td
													...
													className={`${item.expired ? 'debt' : ''} ${
														selectedItem._id === item._id ? 'bg-light' : ''
													}`}
												>
													{item.value}
												</td>
