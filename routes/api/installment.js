      ...
      for (let x = 0; x < initialInstallments.length; x++) {
        ...
        if (initialInstallments[x].student) {
          ...
          if (year >= startYear && year <= endYear) {
            ...
            if (endYear === year) {
              if (endMonth >= number) installments.push(initialInstallments[x]);
            } else installments.push(initialInstallments[x]);
          }
        }
      }

      installments = installments.filter((installment) => installment.student);
    }
...
router.post("/create-list", (req, res) => {
  ...
  for (let x = 0; x < debts.length; x++) {
    ...
    htmlstring += "<tr>" + studentname + installment + year + value + "</tr>";
  }

  const table = "<th>Nombre</th> <th>Cuota</th> <th>Año</th> <th>Valor</th>";
  ...
  pdf
    .create(pdfTemplate(css, img, "movimientos", table, htmlstring), options)
    .toFile(name, (err) => {
      if (err) {
