...
router.get("/", auth, async (req, res) => {
  try {
        ...
        switch (req.query.type) {
          case "Alumno":
            ...
            if (req.query.classroom) {
              if (req.query.classroom === "null") filter.classroom = null;
            ...
            if (req.query.category) {
              ...
              for (let x = 0; x < enrollments.length; x++) {
                ...
                if (enrollments[x].student !== null) {
                  ...
                  user.studentnumber = enrollments[x].student.studentnumber;
              ...
              for (let x = 0; x < students.length; x++) {
                ...
                if (students[x].cel !== undefined) user.cel = students[x].cel;
                if (students[x].dob !== undefined) user.dob = students[x].dob;
                if (enrollment) user.category = enrollment.category.name;
                users.push(user);
              }
            }
            break;
    ...
    if (users.length === 0) {
      return res
        .status(400)
        .json({ msg: "No se encontraron usuarios con dichas descripciones" });
    }
...
router.get("/students/:id", auth, async (req, res) => {
  try {
    ...
    for (let x = 0; x < user.children.length; x++) {
      const student = await User.findOne({ _id: user.children[x].user }).select(
        "-password"
      );
...
router.post("/create-list", (req, res) => {
  ...
  if (usersType === "Alumno") {
    ...
    table = "<th>Nombre</th><th>Email</th><th>Celular</th>";
    switch (usersType) {
      case "Tutor":
      ...
      case "Administrador":
        ...
        if (usersType === "Administrador") table += "<th>Rol</th>";
        break;
      default:
        break;
    }
  ...
  for (let x = 0; x < users.length; x++) {
    ...
    if (usersType === "Alumno") {
      ...
      const email = "<td>" + (users[x].email ? users[x].email : "") + "</td>";
      ...
      if (usersType === "Tutor") {
        const studentname =
          ...
          (users[x].children.length > 0
            ...
            : "") +
          "</td>";
        htmlstring += "<tr>" + name + email + cel + studentname + "</tr>";
      } else {
        const dob =
          "<td>" +
        ...
        if (usersType === "Profesor")
          htmlstring += "<tr>" + name + email + cel + dob + "</tr>";
        else {
          const type = "<td>" + users[x].type + "</td>";
          htmlstring += "<tr>" + name + email + cel + dob + type + "</tr>";
        }
      }
    }
  }
